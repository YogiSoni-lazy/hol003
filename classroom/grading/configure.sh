sudo sed -i "s/RHT_COURSE=.*/RHT_COURSE=hol003/" /etc/rht
sudo sed -i "s/RHT_VERSION_LOCK=.*/RHT_VERSION_LOCK='>=9.0,<10.0'/" /etc/rht
sudo systemctl restart dynolabs-update.service
pip install pip install rht-labs-hol003==9.0.3.dev19 --extra-index-url https://pypi.apps.tools-na.prod.nextcle.com/repository/labs/simple/
lab select hol003
lab --version
